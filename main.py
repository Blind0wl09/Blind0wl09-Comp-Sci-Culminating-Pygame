...
def jumpscare(screen, jumpscare_image, jumpscare_sound, duration=5000, rotation_speed=20, rotation_loops=6):
    ...
    direction = 1  # 1 for rotating right, -1 for rotating left
    initial_time = pygame.time.get_ticks()
    loop_count = 0  # Counter for the number of rotation loops
    ...
    while pygame.time.get_ticks() - initial_time < duration:
        ...
        # Stop rotating if the desired number of loops is reached
        if loop_count >= rotation_loops * 2:  # Each loop is 2 direction changes (left and right)
            angle = 0  # Reset angle to 0 to stop rotation
            direction = 0  # Stop changing direction

        # Rotate the jumpscare image
        rotated_image = pygame.transform.rotate(jumpscare_image, angle)

        # Center the rotated image on the screen
        rotated_rect = rotated_image.get_rect(center=screen.get_rect().center)

        # Clear the screen and draw the rotated image
        screen.fill((0, 0, 0))
...
def night_one():
    ...
    camera_positions = {
        pygame.K_1: pygame.Rect(187, 401, 59, 62),  # Adjust these values based on the map layout
        pygame.K_2: pygame.Rect(422, 403, 57, 59),
        pygame.K_3: pygame.Rect(27, 203, 61, 59),
        pygame.K_4: pygame.Rect(184, 19, 62, 62),
        pygame.K_5: pygame.Rect(498, 42, 64, 61),
        pygame.K_6: pygame.Rect(423, 237, 61, 62),
        pygame.K_7: pygame.Rect(612, 218, 62, 62),
    }
    ...
    jumpscare_image_path1 = "FNAF GAME/IMAGES/JUMPSCARE_ANI1.png"
    jumpscare_image_path2 = "FNAF GAME/IMAGES/JUMPSCARE_ANI2.png"
    jumpscare_image_path3 = "FNAF GAME/IMAGES/JUMPSCARE_ANI3.png"

    if os.path.exists(jumpscare_image_path1): 
        jumpscare_image1 = pygame.image.load(jumpscare_image_path1)
    else:
        print(f"file not found: {jumpscare_image_path1}")
        jumpscare_image1 = None # sets to no image if cant find
    if os.path.exists(jumpscare_image_path2):
        jumpscare_image2 = pygame.image.load(jumpscare_image_path2)
    else:
        print(f"file not found: {jumpscare_image_path2}")
        jumpscare_image2 = None # sets to no image if cant find
    if os.path.exists(jumpscare_image_path3):
        jumpscare_image3 = pygame.image.load(jumpscare_image_path3)
    else:
        print(f"file not found: {jumpscare_image_path3}")
        jumpscare_image3 = None # sets to no image if cant find

    powerout_image_path1 = "FNAF GAME/IMAGES/POWER_OUT1.png"
    powerout_image_path2 = "FNAF GAME/IMAGES/POWER_OUT2.png"
    if os.path.exists(powerout_image_path1): 
        powerout_image1 = pygame.image.load(powerout_image_path1)
    else:
        print(f"file not found: {powerout_image_path1}")
        powerout_image2 = None # sets to no image if cant find
    if os.path.exists(powerout_image_path2):
        powerout_image2 = pygame.image.load(powerout_image_path2)
    else:
        print(f"file not found: {powerout_image_path2}")
        powerout_image2= None # sets to no image if cant find
    ...
    playing_six_am = False
    six_am_start_time = None
    ...
    playing_power_out_sound = False
    playing_power_out_music = False
    ...
    while running:
        ...
        if global_power <= 0 and not power_out_started:
            ...
            animatronic1_probability = 0
            animatronic1_probability_increment = 0
        ...
        if power_out_started:
            if power_out_start_time and pygame.time.get_ticks() - power_out_start_time >= 0:
                if not playing_power_out_sound:
                    ...
                    playing_power_out_sound = True

            if power_out_start_time and pygame.time.get_ticks() - power_out_start_time >= 10000:
                if not playing_power_out_music:
                    power_out_music.play()
                    blink_screen(screen, 0)
                    current_image = powerout_image2
                    playing_power_out_music = True

            if power_out_start_time and pygame.time.get_ticks() - power_out_start_time >= 23000:
                # Call the jumpscare function
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Handle mute key first
                if event.key == pygame.K_m:
                    if phone_channel.get_volume() > 0:  # Check if the sound is not muted
                        phone_channel.set_volume(0)     # Mute the sound
                    else:
            ...
            if animatronic1_current_camera == 2:  # Check if animatronic is at Camera 2
                ...
                if door_open:
                    ...
                    if animatronic1_at_door_time == 0:  # Start the timer if animatronic reaches the door
                        ...
                        animatronic1_probability = 0.0025  # Reset movement probability
    ...
    #animatronic 2 attack logic
        if time_not_looked >= 60000:  # Check if the animatronic can attack
            ...
            if door_open:
                if animatronic2_at_door_time == 0:  # Start the timer if animatronic reaches the door
                    ...
                    jumpscare(screen, jumpscare_image2, jumpscare_sound, duration=3000, rotation_speed=10, rotation_loops=3)
                    running = False  # End the game
            else:
        ...
        if animatronic3_current_camera > 3:  # Jumpscare if animatronic3_current_camera is greater than index 3
            # Call the jumpscare function
            jumpscare(screen, jumpscare_image3, jumpscare_sound, duration=3000, rotation_speed=10, rotation_loops=3)
        ...
        if time_remaining == 0:
            global_power = 1000
            door_states["left"] = True
            door_states["right"] = True
            animatronic3_current_camera = 0
            if not playing_six_am:
                pygame.mixer.stop()
                playing_six_am = True
                six_am_sound.play()
                six_am_start_time = pygame.time.get_ticks()
            ...
            if six_am_start_time and pygame.time.get_ticks() - six_am_start_time >= 7500:
                render_outlined_text(
                    screen,
                    "GET OUT",
                    font,
                    WIDTH // 2 - 80,  #X position 
                    HEIGHT // 1 - 50,  #Y position
                    (255, 255, 255),  #white text
                    (0, 0, 0),  #black outline
                    2,  #outline thickness
                )
        ...
        if animatronic1_current_camera == 2:  # Check if animatronic is at Camera 2
            ...
            if door_open:
                ...
                if animatronic1_at_door_time == 0:  # Start the timer if animatronic reaches the door
                    ...
                    blink_screen(screen, 0)
                    animatronic1_current_camera = 0  # Reset animatronic to Camera 5
                    animatronic1_probability = 0.0075  # Reset movement probability
                    animatronic1_at_door_time = 0  # Reset the door timer
...
animatronic difficulty:
animatronic1_probability = 0.02 
animatronic1_probability_increment = 0.02  

if time_not_looked >= 15000: #animatronic 2

if camera_open_count >= 8:
...
animatronic difficulty:
animatronic1_probability = 0.0025 
animatronic1_probability_increment = 0.0005 

if time_not_looked >= 60000: #animatronic 2

if camera_open_count >= 50: #animatronic 3

"""
def custom_night(global_power_settings, penny_settings, egg_settings, wolfington_settings):
    pygame.init()
    pygame.font.init()
    pygame.mixer.init()

    print(global_power_settings)
    print(penny_settings)
    print(egg_settings)
    print(wolfington_settings)
    # Get the absolute path of the MP3 file
    ambience_mp3 = "FNAF GAME/AMBIENCE.mp3"
    ...
    # Load sound effects (same as before)
    door_sound = pygame.mixer.Sound("FNAF GAME/door slam.mp3")
    camera_sound = pygame.mixer.Sound("FNAF GAME/camera open.mp3")
    camera_switch_sound = pygame.mixer.Sound("FNAF GAME/camera switch.mp3")
    jumpscare_sound = pygame.mixer.Sound("FNAF GAME/jumpscare sound.mp3")
    ...
    animatronic1_camera_sequence = [pygame.K_5, pygame.K_6, pygame.K_2]  # Cameras the animatronic moves through
    animatronic1_current_camera = 0  # Start at the first camera
    animatronic1_probability = penny_settings['probability']
    animatronic1_probability_increment = penny_settings['increment']
    ...
    look_at_time = 0
    time_not_looked = 0
    time_not_looked_threshold = egg_settings['time_not_looked']
    ...
    animatronic3_current_camera = 0
    camera_open_count = 0
    camera_open_threshold = wolfington_settings['camera_open_count']
    ...
    idle_power = global_power_settings['idle_power']
    camera_power = global_power_settings['camera_power']
    door_power = global_power_settings['door_power']
    both_doors_power = global_power_settings['both_doors_power']
    door_bang_power = global_power_settings['door_bang_power']
    vent_flash_power = global_power_settings['vent_flash_power']
...
# Main loop - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    ...
    while running:
        ...
        if camera_open:
            global_power -= camera_power
        if door_states["left"]:
            global_power -= door_power
        if door_states["right"]:
            global_power -= door_power
        if door_states["left"] and door_states["right"]:
            global_power -= both_doors_power
        if vent_flashing:
            global_power -= vent_flash_power
    ...
    if power_out_start_time and pygame.time.get_ticks() - power_out_start_time >= 23000:
        ...
        # Animatronic 1 movement logic 
        if frame_counterANI1 >= 30:
            frame_counterANI1 = 0
            if animatronic1_current_camera < len(animatronic1_camera_sequence):
                if random.random() < animatronic1_probability:
                    blink_screen(screen, 0)
                    animatronic1_probability = penny_settings['probability']
                    animatronic1_current_camera += 1
                else:
    ...
    #animatronic 2 attack logic
        if time_not_looked >= time_not_looked_threshold:
            if not run_sound_playing:
                blink_screen(screen, 0)
                run_sound.play()
                run_sound_playing = True
        ...
        #ANIMATRONIC 3 MOVEMENT
        if animatronic3_current_camera == 0:
            if camera_open_count >= camera_open_threshold:
                animatronic3_current_camera = 1
                camera_open_count = 0
                if not vent_enter_sound_played:
                    vent_enter_sound.play()
